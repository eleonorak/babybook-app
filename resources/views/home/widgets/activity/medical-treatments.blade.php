        ...
        @if($total > 0)
            @foreach($records as $i => $record)
                <tr class="bg-white {{ $total-1 === $i ? 'border-b' : '' }}">
                    ...
                    <td class="px-6 py-4">
                        <span class="block">{{ $record->type->name }}</span>
                        @if($record->vaccines->count() > 0)
                        <span class="block text-gray-400 italic">{{ $record->vaccines->map(fn($item) => sprintf('%s', $item->name) )->join(', ') }}</span>
                        @endif
                    </td>
                    <td class="px-6 py-4">
