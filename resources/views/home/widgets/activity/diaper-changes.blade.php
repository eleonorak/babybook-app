<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <table class="border-t w-full text-sm text-left rtl:text-right text-gray-500">
        ...
        @if($total > 0)
            @foreach($records as $i => $record)
                <tr class="bg-white {{ $total-1 === $i ? 'border-b' : '' }}">
                    <th scope="row" class="flex items-center px-6 py-4 h-full text-gray-900 whitespace-nowrap">
                        <img class="w-10 h-10 rounded-full" src="{{ $record->child->profile_photo }}">
                        <div class="ps-3">
        ...
        @else
            <tr class="bg-white">
                <td class="px-6 py-4" colspan="3">Не се пронајдени резултати</td>
            </tr>
        @endif
        </tbody>
    </table>
