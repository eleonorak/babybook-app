<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <table class="border-t w-full text-sm text-left rtl:text-right text-gray-500">
        ...
        @if($total > 0)
            @foreach($records as $i => $record)
                <tr class="bg-white {{ $total-1 === $i ? 'border-b' : '' }}">
                    ...
                    <td class="px-6 py-4">
                        {{ !empty($record->type->name) ? $record->type->name : 'Непознато' }}
                    </td>
                    <td class="px-6 py-4">
                        {{ !empty($record->value) ? sprintf('%d %s', $record->value, $record->unit->name) : 'Непознато' }}
                    </td>
                    <td class="px-6 py-4">
        ...
        @else
            <tr class="bg-white">
                <td class="px-6 py-4" colspan="5">Не се пронајдени резултати</td>
            </tr>
        @endif
        </tbody>
    </table>
