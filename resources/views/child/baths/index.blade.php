<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex inline-flex items-center gap-4 float-right">
            <x-btn-link-primary href="{{ route('child.baths.create', ['child' => $child]) }}"><span class="babybook-plus"></span> Нова ставка</x-btn-link-primary>
            ...
            <div class="border-l-2 mt-10">
                @if(!$baths->isEmpty())
                    @foreach ($baths as $bath)
                        <div class="bg-[#7c3aed] transform transition cursor-pointer hover:-translate-y-2 ml-10 relative flex items-center px-6 py-4 bg-blue-600 text-white rounded mb-10 flex-col md:flex-row space-y-4 md:space-y-0">
                            ...
                            <a href="{{route('child.baths.edit',['child'=>$child->id,'bath'=>$bath->id])}}" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                                Промени
                            </a>
