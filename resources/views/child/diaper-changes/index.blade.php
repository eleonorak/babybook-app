<x-app-layout>
    <x-slot name="header">
        ...
        <div class="sm:col-span-4 float-right px-2">
            <a href="{{ route('child.show',['child'=>$child->id]) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Назад</a>
        </div>
        <div class="sm:col-span-4 float-right">
            <a href="{{route('child.diaper-changes.create',['child'=>$child])}}"
               class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"> + Промена на пелена</a>
        </div>
    </x-slot>
    ...
    <div class="w-10/12 md:w-7/12 lg:6/12 mx-auto relative py-20">
        ...
        <div class="border-l-2 mt-10">
                ...
                @foreach ($diaperChanges as $diaperChange)
                    <div style="background-color: #7c3aed"
                        ...
                        <div class="flex-auto">
                            ...
                            <h1 class="text-lg">Време : {{$diaperChange->date}}</h1>
                            <h2>Забелешка :  {{$diaperChange->notes}}</h2>
                        </div>
                        <a href="{{route('child.diaper-changes.edit',['child'=>$child->id,'diaper_change'=>$diaperChange->id])}}" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                            Промени
                        </a>
            ...
            <section class="mt-4 text-center">{{ $diaperChanges->links() }}
            </section>
